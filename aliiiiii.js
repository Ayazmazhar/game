...
// --- PARTICLE FX MANAGER (Crash Explosions & Score Scatter) ---
class ParticleFXManager {
    ...
    draw(ctx, cellW, cellH) {
        ...
        this.scorePopups.forEach(sp => {
            ...
            ctx.shadowBlur = 12;
            ctx.font = `bold ${Math.max(14, Math.round(cellH * 0.7))}px Outfit, sans-serif`;
...
class AudioManager {
    ...
    playBonusEatSFX() {
        ...
        try {
            if (this.audioCtx.state === 'suspended') this.audioCtx.resume();
            const now = this.audioCtx.currentTime;
            const notes = [523.25, 659.25, 784.88];
    ...
    playCrashSFX() {
        ...
        try {
            ...
            const bufferSize = Math.round(this.audioCtx.sampleRate * 0.25);
